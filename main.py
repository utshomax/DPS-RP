...
class DiseasePrediction:
    ...
    def __init__(self, model_name=None):
        ...
        self._feature_correlation(data_frame=self.train_df, show_fig=True)
        # Model Definition
    ...
    def _feature_correlation(self, data_frame=None, show_fig=False):
        ...
        sn.heatmap(corr, square=True, annot=False, cmap="Blues")
    ...
    def train_model(self):
        ...
        sn.heatmap(conf_mat, annot=False, cmap="Blues")
        plt.tight_layout()
        plt.title('Confusion Matrix - {}'.format(self._getModelName()))
        plt.show()
        # Model Classification Report
        clf_report = classification_report(y_val, y_pred,output_dict=True)
        df = pd.DataFrame(clf_report).transpose()
        df.to_csv('report- {}'.format(self._getModelName()))
...
if __name__ == "__main__":
    ...
    dp1 = DiseasePrediction(model_name=current_model_name)
    # Train the Model
    dp1.train_model()
    # Get Model Performance on Test Data
    test_accuracy1, classification_report_var = dp1.make_prediction(saved_model_name=current_model_name)
    print("Model Test Accuracy: ", test_accuracy1)
    algo_acc.append(test_accuracy1)
    current_model_name = 'decision_tree'
    # Instantiate the Class
    dp2 = DiseasePrediction(model_name=current_model_name)
    # Train the Model
    dp2.train_model()
    # Get Model Performance on Test Data
    test_accuracy2, classification_report_var = dp2.make_prediction(saved_model_name=current_model_name)
    print('classification Report',classification_report_var)
    print("Model Test Accuracy: ", test_accuracy2)
    algo_acc.append(test_accuracy2)
    current_model_name = 'random_forest'
    # Instantiate the Class
    dp3 = DiseasePrediction(model_name=current_model_name)
    # Train the Model
    dp3.train_model()
    # Get Model Performance on Test Data
    test_accuracy3, classification_report_var = dp1.make_prediction(saved_model_name=current_model_name)
    algo_acc.append(test_accuracy3)
    print("Model Test Accuracy: ", test_accuracy3)

    current_model_name = 'gradient_boost'
    # Instantiate the Class
    dp4 = DiseasePrediction(model_name=current_model_name)
    # Train the Model
    dp4.train_model()
    # Get Model Performance on Test Data
    test_accuracy4, classification_report_var = dp1.make_prediction(saved_model_name=current_model_name)
    algo_acc.append(test_accuracy4)
    print("Model Test Accuracy: ", test_accuracy4)
    lavels = ['Naive Bays','Decision Tree','Random Forest','Gredient Decent']
    print(algo_acc)
    plt.title('Comparisions')
    plt.bar(lavels,algo_acc)
    plt.show()
